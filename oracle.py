...
def createtable():
    query = '''create table suresh123456 (id number(2) primary key,name varchar(50))'''
    cur.execute(query) 
def insertrecord(sid,name):
    record={'id':str(sid),'name':name}
    cur.execute("insert into suresh123456(id,name) values(:id,:name)",record)
    conn.commit()

def read_records():
    query = 'select * from suresh123456'
    cur.execute(query)
...
def update_name(sid,name):
    ...
    record={'id':str(sid),'name':name}
    query='update suresh123456 set name=:name where i =:id'
...
def delete_record(sid):
    record={'id':str(sid)}
    query='delete from suresh123456 where id =:id'
    cur.execute(query,record)
    cur.commit()

def truncate():
    query ='truncate table suresh123456'
    cur.execute(query)
